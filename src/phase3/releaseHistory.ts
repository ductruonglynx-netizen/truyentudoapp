...
export const WRITER_RELEASE_NOTES: ReleaseNote[] = [
  {
    ...
    items: [
      'Nâng cấp toàn bộ quy trình dịch theo hướng an toàn cho file lớn: chia bước rõ ràng, có mốc lưu tiến độ, tự phục hồi khi quá tải và giữ mạch nhất quán giữa các chương.',
      ...
      'Bổ sung bảng tinh chỉnh sinh văn bản chi tiết để người dùng tự chọn mức sáng tạo, độ dài phản hồi và nhịp phản hồi theo từng tác vụ.',
      ...
      'Mở rộng lựa chọn dịch vụ AI và mẫu AI, giúp bạn chọn đúng loại phù hợp cho dịch, viết, phân tích hoặc tạo ảnh một cách linh hoạt hơn.',
  ...
  },
  {
    ...
    items: [
      ...
      'Cải thiện nhịp đồng bộ tự động theo hướng mượt hơn: khi có nhiều thay đổi liên tiếp, app gộp đợt để giảm rung lắc giao diện.',
    ],
  },
  {
    version: '0.1c',
    ...
    items: [
      ...
      'Mỗi truyện có một mã liên kết riêng để đường dẫn ổn định hơn và hạn chế nhầm lẫn giữa các truyện tên gần giống.',
      ...
      'Hoàn thiện xử lý liên kết sai và giữ khả năng mở lại liên kết cũ để tránh đứt mạch đọc.',
    ],
  },
  {
    version: '0.1b',
  ...
  },
  {
    ...
    items: [
      'Thêm Trung tâm sao lưu mới: có nút Sao lưu ngay, lịch sử sao lưu ngay trong app, khôi phục theo từng mốc thời gian và cảnh báo đỏ khi để quá lâu chưa sao lưu.',
      'Chuyển hướng từ đồng bộ tự động rủi ro sang đồng bộ thủ công: dữ liệu tài khoản giờ chỉ đồng bộ khi người dùng tự bấm, giúp tránh ghi đè âm thầm trong nền.',
      'Tạo cơ chế sao lưu cục bộ nhiều mốc và cho phép đẩy file sao lưu lên Google Drive sau mỗi lần lưu dữ liệu khi người dùng đã kết nối Drive.',
      'Chuyển lưu trữ dữ liệu tài khoản sang hệ thống máy chủ thống nhất hơn để truyện, nhân vật, từ điển dịch và cấu hình cá nhân được đồng bộ ổn định hơn.',
      'Bổ sung bộ mẫu cấu hình cơ sở dữ liệu để việc triển khai lưu trữ mới rõ ràng, dễ kiểm tra và ít phụ thuộc cấu hình cũ.',
      'Nâng màn hình tiến trình AI để hiển thị rõ từng giai đoạn xử lý, tiến độ thực tế và phần việc đang chạy thay vì chỉ có vòng xoay.',
      'Làm hệ thống thông báo bớt ồn hơn: gom nhóm toast trùng lặp, giới hạn số lượng hiển thị và cho phép đóng tay khi cần.',
      'Sửa Kho Prompt theo hướng dễ tin cậy hơn: có trạng thái Chưa lưu/Đã đồng bộ, tự động lưu khi đổi mục hoặc đóng bảng, đồng thời nút Lưu chỉ sáng khi thực sự có thay đổi.',
      'Làm rõ trang Công cụ là bộ công cụ cục bộ: gắn nhãn không gọi AI, đổi tên các thao tác dễ gây hiểu nhầm và thêm lối đi rõ ràng sang luồng AI nâng cao.',
      ...
      'Bổ sung tự động lưu dữ liệu cục bộ vào tài khoản đã đăng nhập, bao gồm hồ sơ giao diện, từ điển dịch, văn mẫu, kho prompt và cấu hình ngân sách AI.',
      ...
      'Tích hợp luồng tạo ảnh bìa chạy nền ngay trong app: bấm tạo bìa là gửi yêu cầu trực tiếp, chờ kết quả trả về ngay trong giao diện, không bật popup hay tab ngoài.',
  ...
  },
  {
    ...
    items: [
      'Nâng cấp hướng dẫn Writer Pro để bám chặt mục tiêu chương, mốc thời gian và thuật ngữ trước khi sinh nội dung.',
      'Tự động khóa thuật ngữ sau khi AI trả về, giảm tình trạng tên riêng và từ khóa bị trôi lại về bản gốc.',
      'Bổ sung cảnh báo độ sẵn sàng ngữ cảnh khi dữ liệu đầu vào còn thiếu, giúp AI hoạt động ổn định hơn.',
      'Cải thiện tạo ảnh bìa: tự dựng mô tả ảnh tốt hơn và có bìa dự phòng rõ bố cục khi dịch vụ ảnh AI bị lỗi.',
      ...
      'Sửa Kho Prompt để thao tác Lưu thay đổi được giữ bền vững, đồng thời các nút trong trang Công cụ giờ đã có hành vi thực tế thay vì chỉ hiển thị cho có.',
      'Siết an toàn sao lưu/khôi phục: loại thông tin nhạy cảm khỏi file sao lưu, kiểm tra dữ liệu khi nhập và lưu sẵn một bản an toàn trước khi khôi phục.',
      'Khắc phục 2 mục trong Writer Pro: tab Tone và Context giờ cập nhật đúng ngữ cảnh, đồng thời cho phép đẩy kết quả về vùng làm việc nhanh hơn.',
